...
- **Mobile Smart-Download & Selection Logic**: Implemented a hybrid strategy to balance gallery access and stability. iOS selections ≤ 20 use the native **Web Share API** for Photos integration, while Android and large iOS batches utilize **ZIP Archives** for a reliable "Save to Gallery" path via extraction. This is supported by an **Instructional HUD** and **Selection Safety** logic that standardizes all IDs to Strings to prevent type-mismatch bugs. Selection is automatically cleared post-download for a cleaner UX.
- **Tunnel Resilience & Auto-Recovery**: The bot runs a periodic background task `keep_tunnel_alive` (every 5 minutes) using `discord.ext.tasks` to ensure that both the dashboard and the Cloudflare Tunnel remain active and healthy. This automatically recovers the public link if the `cloudflared` process crashes, gets updated by Cloudflare, or shuts down due to network interruptions.
