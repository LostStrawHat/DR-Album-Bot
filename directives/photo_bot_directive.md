...
- **Web Share API & Mobile OOM Protection**: Optimized the mobile download experience by utilizing `navigator.share()` for direct gallery integration. To protect device RAM, we implemented a 15-item batching limit and sequential `File` object generation, ensuring massive vaults remain manageable on smartphones.
